 ...
 * @author versley
 */
public class LibSVMClassifierFactory implements ClassifierFactory {

    private static LibSVMClassifierFactory _factory;

    public static LibSVMClassifierFactory getInstance() {
        if (_factory == null) {
            _factory = new LibSVMClassifierFactory();
        }
        return _factory;
    }
    ...
    public void do_learning(String model_name, String options, String learner) throws IOException {
        try {
            svm.do_estimation(
                    new File(ConfigProperties.getInstance().getModelDir(),
                    model_name).getAbsolutePath(),options);
    ...
    public OfflineClassifier getClassifier(String model_name, String options, String learner) throws IOException {
        try {
            return new SVMClassifierBinary(new File(ConfigProperties.getInstance().getModelDir(),
                            model_name).getAbsolutePath());
        } catch (ClassNotFoundException ex) {
