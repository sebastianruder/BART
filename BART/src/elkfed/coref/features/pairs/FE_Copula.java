...
// exclude "there is .." (ToDo: exclude other expletives!)
      ...
      if (inst.getAntecedent().getMarkableString().toLowerCase().matches(NONREF_NP)) return false;

//exclude date and time

if (inst.getAnaphor().getSemanticClass()==SemanticClass.TIME) return false;
if (inst.getAnaphor().getSemanticClass()==SemanticClass.DATE) return false;
if (inst.getAntecedent().getSemanticClass()==SemanticClass.TIME) return false;
if (inst.getAntecedent().getSemanticClass()==SemanticClass.DATE) return false;
...
// should be subj-obj of the same verb
      ...
      if (!vp.value().equalsIgnoreCase("vp")) return false;
      while(vp.parent(sentenceTree)!=null && vp.parent(sentenceTree).value().equalsIgnoreCase("vp")) {vp=vp.parent(sentenceTree);}
      if (vp==null) return false;
      if (vp.parent(sentenceTree)==null) return false;

      Boolean foundante=false;
      Tree[] chldsup=vp.parent(sentenceTree).children();
      for (int i=0; i<chldsup.length; i++) {
       if (chldsup[i]==AnteTree) foundante=true;
       if (chldsup[i]==vp && foundante==false) return false;
      }
...
// should not contain a modal verb
      ...
      for (int i=0; i<chlds.length; i++) {
       if (chlds[i].value().equalsIgnoreCase("rb")) return false;
