...
public class Mention implements Comparable<Mention> {
	...
	public void SetNumber() {
		if (getParseHead() == null) {
			...
			if (getHeadString().toLowerCase().matches(PRONOUN)) {
				issing = getHeadString().toLowerCase().matches(
						SINGULAR_PRONOUN_ADJ);
			} else {
	...
	public Mention(Markable markable, MiniDiscourse doc) {
		LanguagePlugin lang_plugin = ConfigProperties.getInstance()
				.getLanguagePlugin();
		_markable = markable;
		_document = doc;
	...
	public boolean getProperName() {
		return _mentionType.features
				.contains(MentionType.Features.isProperName);
	}
	...
	public boolean getPossPronoun() {
		return _mentionType.features
				.contains(MentionType.Features.isPossPronoun);
	}
	...
	public boolean getPersPronoun() {
		return _mentionType.features
				.contains(MentionType.Features.isPersPronoun);
	}
	...
	public boolean getDnewDeterminer() {
		return _mentionType.features
				.contains(MentionType.Features.isDnewDeterminer);
	}

	public boolean getIndefinite() {
		return _mentionType.features
				.contains(MentionType.Features.isIndefinite);
	}

	public boolean getDemonstrative() {
		return _mentionType.features
				.contains(MentionType.Features.isDemonstrative);
	}

	public boolean getDemPronoun() {
		return _mentionType.features
				.contains(MentionType.Features.isDemPronoun);
	}

	public boolean getDemNominal() {
		return _mentionType.features
				.contains(MentionType.Features.isDemNominal);
	}
	...
	public boolean getIsFirstSecondPerson() {
		return _mentionType.features
				.contains(MentionType.Features.isFirstSecondPerson);
	}
	 ...
	 */
	/* public String getHeadString(){return headString;} */// Olga: use prs
															// first (cf. above)
	 ...
	 * @return mention head POS of NP
	 */
	/* public String getHeadPOS(){return headPos;} */// Olga: commented -- use
														// prs first (cf. new
														// getHeadPos())
	...
	private Tree getSentenceTreeWithDiscurseIdsInLeaves(Tree sentTree) {
		...
		if (discElements.length != leaves.size()
				|| discIDs.length != leaves.size()) {
			throw new RuntimeException(
					"SentenceTree leaves and discourse elements do not match for mention "
	...
	public void setParseInfo(Tree sentTree, int start, int end) {
		...
		LanguagePlugin lang_plugin = ConfigProperties.getInstance()
				.getLanguagePlugin();
		List<Tree>[] parseInfo = lang_plugin.calcParseInfo(sentTree, start,
				end, _mentionType);
		...
		Tree[] parseExtra = lang_plugin.calcParseExtra(sentTree, start, end,
				_ParseHead, getStHeadFinder());
	...
	public boolean overlapsWith(Mention m) {
		...
		if (m1.getRightmostDiscoursePosition() <= m2
				.getLeftmostDiscoursePosition())
			return false;
		else if (m2.getRightmostDiscoursePosition() <= m1
				.getLeftmostDiscoursePosition())
			return false;
		else
			return true;
	...
	public boolean embeds(Mention m) {
		Markable m1 = getMarkable();
		Markable m2 = m.getMarkable();
		return m1.getLeftmostDiscoursePosition() <= m2
				.getLeftmostDiscoursePosition()
				&& m1.getRightmostDiscoursePosition() >= m2
	...
	public static String getMarkableString(final Markable markable) {
		return new StringBuffer(markable.toString())
				.deleteCharAt(markable.toString().length() - 1).deleteCharAt(0)
				.toString();
	}
	...
	public boolean isPrenominal() {
		return Boolean.parseBoolean(getMarkable().getAttributeValue(
				ISPRENOMINAL_ATTRIBUTE));
	}
	...
	public String computeAppType(Mention np) {
		...
		Tree h0 = highestProjection.children()[0]
				.headPreTerminal(getStHeadFinder());
		Tree h2 = highestProjection.children()[2]
				.headPreTerminal(getStHeadFinder());
		if (h0 == null)
			return headAppo;
		if (h2 == null)
		...
		if (highestProjection.children()[0] == lowestProjection
				&& highestProjection.children()[2].label().toString()
						.equals("NP") && !h0.label().toString().equals("NNP")
				&& h2.label().toString().equals("NN"))

			return highestProjection.children()[2].headTerminal(
					getStHeadFinder()).toString();
		...
		if (highestProjection.children()[0].label().toString().equals("NP")
			...
			return highestProjection.children()[0].headTerminal(
					getStHeadFinder()).toString();
	 ...
	 */
	public Tree getMarkableTree(int leftMostDiscursePos,
			int rightMostDiscursePos) {
	 ...
	 *         inside the markables subtree with the given postag
	 */
	public ArrayList<String> getHighestProjectingPhraseWithPOS(
			Tree markableSubtree, String postag) {
	...
	public String[] getSentenceDiscourseElements() {
		return _document.getDiscourseElements(getSentenceStart(),
				getSentenceEnd());
	}
	...
	public String[] getSentenceDiscourseElementIDs() {
		return _document.getDiscourseElementIDs(getSentenceStart(),
				getSentenceEnd());
	}
	...
	public List<String> getDiscourseElementsByLevelAndExtendedSpan(
			String markableLevel, int spanleft, int spanright) {
		List<String> words = new ArrayList<String>();
		int from;
		int to;
		MarkableLevel level = _document.getMarkableLevelByName(markableLevel);
	...
	public Markable[] getSentenceMarkables(MarkableLevel level) {
		return level.getMarkablesAtSpan(this.getSentenceStart(),
				this.getSentenceEnd());
	}
	...
	public Tree getMarkableSubTree() {
		return getMarkableTree(this.getMarkable()
				.getLeftmostDiscoursePosition(), this.getMarkable()
				.getRightmostDiscoursePosition());
	}
	...
	public String computePredicationType(Mention np) {
		...
		if (!(parentNode == null) && parentNode.children().length > 1
			...
			String hword10 = parentNode.children()[1].children()[0]
					.headTerminal(new ModCollinsHeadFinder()).toString();
			if (hword10.equals("is") || hword10.equals("are")
					|| hword10.equals("was") || hword10.equals("were")) {
				Tree pchild11 = parentNode.children()[1].children()[1];
				if (pchild11 != null) {// &&
					if (pchild11.label().toString().equals("NP")) {
						String pchild11_headpos = pchild11
								.headPreTerminal(new ModCollinsHeadFinder())
								.label().toString();
						if (!pchild11_headpos.equals("JJS")
								&& !pchild11_headpos.equals("NNP")) {
							predType = pchild11.headTerminal(
									new ModCollinsHeadFinder()).toString();
						}
					}
				}
			}
	...
	public String computePredicationAttr(Mention np) {
		...
		if (!(parentNode == null) && parentNode.children().length > 1
				&& parentNode.children()[1].label().toString().equals("VP")
				&& parentNode.children()[1].children().length > 1) {
			if (parentNode.children()[1].children()[0]
					.headTerminal(new ModCollinsHeadFinder()).toString()
					.equals("is")
					|| parentNode.children()[1].children()[0]
							.headTerminal(new ModCollinsHeadFinder())
							.toString().equals("are")
					|| parentNode.children()[1].children()[0]
							.headTerminal(new ModCollinsHeadFinder())
							.toString().equals("was")
					|| parentNode.children()[1].children()[0]
							.headTerminal(new ModCollinsHeadFinder())
							.toString().equals("were")) {
				if (!(parentNode.children()[1].children()[1] == null)) {// &&
					if (parentNode.children()[1].children()[1].label()
							.toString().equals("ADJP")) {
						predAttr = parentNode.children()[1].children()[1]
								.headTerminal(new ModCollinsHeadFinder())
								.toString();
						// System.out.println("ATTR!!! " + predAttr);
					} else if (parentNode.children()[1].children()[1].label()
							.toString().equals("NP")
							&& parentNode.children()[1].children()[1]
									.headPreTerminal(new ModCollinsHeadFinder())
	 ...
	 */
	public String getJoinedStringFromDiscIds(ArrayList<String> discIds,
			String markableLevel) {
		if (discIds != null) {
			int from = MarkableHelper.parseId(discIds.get(0), "word");
			int to = MarkableHelper.parseId(discIds.get(discIds.size() - 1),
					"word");
	 ...
	 * @return a joined string of the markable levels tag attribute
	 */
	public String getJoinedStringFromDiscIds(int from, int to,
			String markableLevel) {
		MarkableLevel level = _document.getMarkableLevelByName(markableLevel);
