 ...
 * @author Julian
 */
public class SieveDiscourseEntity {
	 ...
	 */
	public SieveDiscourseEntity(Mention m) {
		ID = nextID;
		nextID++;
	...
	public void merge(Mention ante) {
		...
		SieveDiscourseEntity deAnte = ante.getSieveDiscourseEntity();
		if (deAnte == this) {
			return;
		}
		for (Mention m : deAnte.getMentions()) {
			mentions.add(m);
			m.setSieveDiscourseEntity(this);
			addWords(m);
		}
