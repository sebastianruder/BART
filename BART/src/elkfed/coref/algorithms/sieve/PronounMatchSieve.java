...
/**
 * This Sieves finds antecedents for Pronouns. First we find all compatible
 * Mentions that are at most 3 sentences apart from the anapher. Then we rank
 * them according to their salience and return the most salient antecedent.
 * 
 * @author Julian, Xenia
 * 
 */
public class PronounMatchSieve extends Sieve {
	...
	private List<Mention> getAntecedents(Mention m) {
		...
		for (int idx = 0; idx < mentions.indexOf(m); idx++) {
			...
			if (m.getReflPronoun()
					&& !FE_Syntax_Binding.getAnaBoundInBindingDomain(pair)) {
				continue;
			}
			if (m.getPersPronoun()
					&& !m.getHeadPOS().equalsIgnoreCase("pposat")
					&& FE_Syntax_Binding.getAnaBoundInBindingDomain(pair)) {
				continue;
			}
	...
	/**
	 * Scores the Salience of the pairInstance: +100 if the antecedent is a
	 * named entity +20 if mention and antecedent are in the same Sentence +170
	 * if the antecedent is the Subject of its clause +70 if the antecedent is
	 * the Accusative Object of its clause +50 if the antecedent is the Dative
