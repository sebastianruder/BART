...
/**
 * Implements the decodeDocument method which walks through the document and
 * runs the appropriate sieve for every mention.
 * 
...
public class SieveDecoder implements CorefResolver {
	...
	public DisjointSet<Mention> decodeDocument(List<Mention> mentions,
		...
		_logger.log(Level.INFO, String.format(
				"%s: decode document with %d mentions\n", getClass()
						.getSimpleName(), mentions.size()));
		...
		for (int walk_through = 1; walk_through < 11; walk_through++) {
			...
			for (int i = 0; i < mentions.size(); i++) {
				...
				/*
				 * ante_idx is the index of the antecedent; if no-one is found,
				 * it is set to -1 by the sieve
				 */
